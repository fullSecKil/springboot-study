...
//利用ps完成数据的修改（修改第一条，除主键之外任意字段值）
public class PreUpdate {
	public void preUpd() throws Exception{
		//若干变量、对象
		...
		String sql="update emp set name=?,sex=?,job=?,sal=?,bonus=? where id=?";//增加数据语句，预设占位符
		...
		if (flag>0) {
			...
			System.out.println("修改了"+flag+"条记录");	
		} else {
