 ...
 *
 * @Api： 描述Controller
 * @ApiIgnore: 忽略Controller,指不对当前类做扫描
 * @ApiOperation 描述Controller类中的method接口
 *
 * @ApiImplicitParams 描述多个输入参数信息
 * @ApilmplicitParam 描述单个入参信息
 */
@RestController
@RequestMapping("/user")
@Api(tags="1.1", description = "用户管理", value = "用户管理")
//tags 标签，description 描述，
public class UserController {
    private static final Logger log = LoggerFactory.getLogger(UserController.class);

    @GetMapping
    @ApiOperation(value = "条件查询(DONE)")
    @ApiImplicitParams({@ApiImplicitParam(name="username", value = "用户名", dataType = ApiDataType.STRING, paramType = ApiParamType.QUERY),
    ...
    public User post(@RequestBody User user){
        log.info("如果是POST PUT 这种@RequestBody的可以不用写@ApiImplicitParam");
        return user;
    }
